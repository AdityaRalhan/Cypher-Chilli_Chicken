"use client"
import { Eye, Settings, ChevronRight, BotMessageSquare , Footprints, Workflow} from "lucide-react"
...
const [dropdownOpen, setDropdownOpen] = useState(false);
  ...
  const handleSignOut = async () => {
    ...
    await signOut({ callbackUrl: "/" }); // Redirect to homepage or login page
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="flex items-center space-x-2">
            <BotMessageSquare className="text-blue-400 h-8 w-8" />
            ...
            {session?.user ? (
              <>
                ...
                <img
                  ...
                  alt="Profile"
                  className="h-10 w-10 rounded-full cursor-pointer border border-blue-400"
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {portalOptions.map((option, index) => (
            ...
            >
              <Link href={`${option.title.toLowerCase()}`} className="block group">
                <div
